...
const MagicBento: React.FC<BentoProps> = ({
  ...
  return (
    <>
      ...
      <BentoCardGrid ref={gridRef} className={gridClassName}>
        {items.map((card, index) => {
          // Changed from cardData to items
...
const ParticleCard: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (enableMagnetism) {
        const magnetX = (x - centerX) * 0.15; //changed
...
const BentoCardGrid = React.forwardRef<
  HTMLDivElement,
  { children: React.ReactNode; className?: string }
>(({ children, className = "" }, ref) => (
  <div className={`card-grid bento-section ${className}`} ref={ref}>
    {children}
  </div>
));
